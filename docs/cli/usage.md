...
| --- | --- | --- |
| `pull` | 擷取每日 bars（單 symbol 或 universe） | `--ticker` / `--universe-file` / `--start` / `--end` / `--adjusted` / `--preview` / `--out` / `--out-dir` |
| `quote` | 擷取即時 quote 快照 | `--tickers` (CSV) / `--preview` / `--out` / `--out-dir` |
...
1. **務必先 preview**：正式輸出檔案前使用 `--preview`
