...
Breaker identity 由 upstream authority 加上 optional logical group 組成；未標記 caller 維持 host-level 行為。Yahoo requests 在每個 authority 下使用固定 group：`yahoo-auth`、`yahoo-chart`、`yahoo-timeseries`、`yahoo-options`、`yahoo-news`、`yahoo-web`。當某個 identity 在 rolling window 內達到最小樣本數及失敗率閾值，只有該 authority + group 進入 open 狀態，直到 `reset_timeout_ms` 屆滿。

Auth 或 scrape circuit 可以開路而不拒絕同 host 的健康 chart 或 news request；`429` 與 `5xx` 仍是各自 family 內的 breaker failure。當下行為：
