...
The tool supports FX conversion preview, bus publishing, and local export.`,
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		// binary imports `log` above via the file-scope alias below), so any
		// APP_LOG_LEVEL set on the process environment is honored at startup.
...
func Execute() error {
	// Wire gosdk/metric cobra hook once — it emits a command_line_trigger
	// metric for every subcommand execution, then chains our PersistentPreRunE.
