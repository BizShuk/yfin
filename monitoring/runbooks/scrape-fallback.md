...
3. **Force API Mode**
    ...
    ```bash
    # Single ticker
    yfin scrape --config config/prod.yaml \
      --fallback api-only \
      --ticker AAPL \
      --endpoint quote \
      --preview
...
yfin scrape --force --ticker AAPL --endpoint profile --preview --config config/dev.yaml
...
yfin scrape --config config/last-known-good.yaml --ticker AAPL --endpoint key-statistics
