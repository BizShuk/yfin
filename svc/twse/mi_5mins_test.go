...
func TestFetchMI_5MINS_Decode(t *testing.T) {
	...
	raw, err := FetchMI_5MINS(context.Background(), client, "20260620", url.Values{})
	if err != nil {
		t.Fatalf("FetchMI_5MINS returned error: %v", err)
	}
	resp, ok := raw.(model.MI_5MINSResponse)
	if !ok {
		t.Fatalf("expected model.MI_5MINSResponse, got %T", raw)
	}
	if resp.GetStat() != "OK" {
...
func TestFetchMI_5MINS_NoData(t *testing.T) {
	...
	client := newTestClient(t, srv)
	_, err := FetchMI_5MINS(context.Background(), client, "20260620", url.Values{})
	if err == nil {
