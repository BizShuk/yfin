...
func TestFetchT86_Decode(t *testing.T) {
	...
	raw, err := FetchT86(context.Background(), client, "20260620", url.Values{})
	if err != nil {
		t.Fatalf("FetchT86 returned error: %v", err)
	}
	resp, ok := raw.(model.T86Response)
	if !ok {
		t.Fatalf("expected model.T86Response, got %T", raw)
	}
	if resp.GetStat() != "OK" {
...
func TestFetchT86_NoData(t *testing.T) {
	...
	client := newTestClient(t, srv)
	_, err := FetchT86(context.Background(), client, "20260620", url.Values{})
	if err == nil {
